    ...
    <page-box>
        <view class="order-alipay">
            <view class="alipay-guide">
                ...
                <view class="tips" v-if="isWeixin">点击右上角"..."用浏览器打开进行快速支付。</view>
                <textarea
                        readonly
                        type="text"
                        id="input"
...
export default {
    ...
    mounted() {
        ...
        console.error(this.$Route.query.url_alipay,'支付情况')

        // 微信公众号环境支付宝付款直接跳转
        if(!$utils.is_weixin()) {
            window.location.href = this.$Route.query.url_alipay;
            return
        }
        this.url = decodeURI(this.$Route.query.url_alipay) // 防止微信公众号使用浏览器打开url编码问题
        if (this.$Route.query.orderID == '1') {
            this.orderId = this.$Route.query.id.split() || this.orderId;
        } else {
    ...
    methods: {
        ...
        paySuccess() {
            if (this.pageFrom === 'coupunPay') {
                ...
                this.$api.orderApi.orderPayResult(obj, { errorToast: false }).then((res) => {
                    if (res.error == 0) {
                        this.paySuccessFlag = true
                        this.$toast('支付成功')
                        this.jumpGoPage()
