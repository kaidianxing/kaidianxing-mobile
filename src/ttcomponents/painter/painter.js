  ...
  methods: {
    ...
    startPaint() {
      ...
      if (!(getApp().systemInfo && getApp().systemInfo.screenWidth)) {
        try {
          getApp().systemInfo = tt.getSystemInfoSync();
        } catch (e) {
      ...
      this.downloadImages().then((palette) => {
        ...
        const ctx = tt.createCanvasContext('k-canvas', this);
    ...
    saveImgToLocal() {
      ...
      setTimeout(() => {
        tt.canvasToTempFilePath({
          ...
          fail: function (error) {
            console.error(`canvasToTempFilePath failed tt1, ${JSON.stringify(error)}`);
            that.triggerEvent('imgErr', {
              error: error
            });
          },
    ...
    getImageInfo(filePath) {
      console.log(filePath, 'pppppppppppppppppppppppppppppppppp');
      const that = this;
      tt.getImageInfo({
        src: filePath,
        success: (infoRes) => {
