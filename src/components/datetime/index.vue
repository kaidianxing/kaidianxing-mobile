	...
	<view name='KXDateTime'>
		...
		<uni-popup ref="popup" type="bottom">
			<view class="but">
				...
				<text @click="ok" class="theme-primary-color">确定</text>
			</view>
...
<script>
	import uniPopup from './uni-popup/index.vue'
	export default {
		name: 'KXDateTime',
		components: {
			uniPopup
		...
		methods: {
			open() {
				...
				// 默认赋值，如果date有值，默认展示date的值
                // this.valueStr = this.getValueStr(this.date);
				this.$forceUpdate();
				if (this.valueStr) {
            ...
            getValueStr(date) {
			    ...
			    let yearNum = this.years.indexOf(date.getFullYear());
			    let monthNum = this.months.indexOf(date.getMonth() + 1);
			    let dayNum = this.days.indexOf(date.getDate());
			    let hoursNum = this.hours.indexOf(date.getHours());
			    let minutesNum = this.mins.indexOf(date.getMinutes());
			    return JSON.stringify([yearNum, monthNum, dayNum, hoursNum, minutesNum]);
            }
		},
		watch: {
			year() {
